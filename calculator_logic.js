...
function updateOrderButton() {
  const orderBtnWrapper = document.getElementById('order-btn-wrapper');
  const orderBtnText = document.getElementById('order-btn-text');
  
  if (selectedItems.base) {
    orderBtnWrapper.classList.add('enabled');
    orderBtnText.textContent = 'Order Now';
    orderBtnWrapper.onclick = function() {
      alert('Order functionality will be implemented soon!\n\nYour selection:\n' + 
            JSON.stringify(selectedItems, null, 2));
    };
  } else {
    orderBtnWrapper.classList.remove('enabled');
    orderBtnText.textContent = 'Select a Base to Order';
    orderBtnWrapper.onclick = null;
  }
}
